...
# all(): 반복 가능한(iterable) 자료형을 입력인수로 넣고,
#        모두 참이면 True, 하나라도 거짓이면 False를 돌려주는 함수
#        반복 가능한 자료형 -> 리스트, 튜플, 문자열, 딕셔너리, 집합 등

# any(): 반복 가능한(iterable) 자료형을 입력인수로 넣고,
#        하나라도 참이 있으면 True, 모두 거짓일 경우에만 False를 돌려주는 함수
#        반복 가능한 자료형 -> 리스트, 튜플, 문자열, 딕셔너리, 집합 등
...
# filter(): 첫 번째 인수로 함수이름, 두번째 인수로 그 함수에 차례로 들어갈 
#           반복 가능한 자료형을 받아 그 함수에 차례로 대입하면서 입력되었을때 
#           참인 것만 묶어서 돌려주는 함수
...
# map(): 함수(f)와 반복 가능한(iterable) 자료형을 입력받아, 
#        입력된 자료형의 각 요소가 함수 f에 의해 수행된 결과를 묶어서 돌려주는 함수
